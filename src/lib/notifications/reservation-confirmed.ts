...
import { getReservationAccessCode } from "@/lib/reservations/reservation-access-code";
...
export async function sendReservationConfirmedEmail(
  ...
  const accessCode = getReservationAccessCode(params.reservationNumber);
    ...
    html: `
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:560px;font-family:system-ui,-apple-system,sans-serif;font-size:15px;line-height:1.5;color:#0f172a">
        ...
        <tr><td style="padding:16px 0;border-top:1px solid #e2e8f0;border-bottom:1px solid #e2e8f0">
          ...
          <p style="margin:0;font-size:12px;color:#64748b">Código de 6 dígitos (validación manual en acceso si no leen el QR):</p>
          <p style="margin:6px 0 0;padding:10px 12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:4px;font-family:ui-monospace,monospace;font-size:18px;font-weight:700;letter-spacing:0.2em;color:#0f172a">${accessCode}</p>
        </td></tr>
