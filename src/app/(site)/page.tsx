...
import { fetchPublicEvents, fetchPublicTickets } from "@/features/events/server/queries";

export const dynamic = "force-dynamic";

export default async function HomePage(): Promise<JSX.Element> {
  const [publicTickets, publicEvents] = await Promise.all([fetchPublicTickets(), fetchPublicEvents()]);
  const featuredEvent = publicEvents[0];
  const eventDetailHref = featuredEvent ? `/events/${featuredEvent.slug}` : "/events";
  ...
  return (
    <div className="space-y-14 md:space-y-16">
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-4 sm:p-6 md:p-10">
            <div className="max-w-2xl space-y-4 sm:space-y-5">
              ...
              <p className="text-sm leading-relaxed text-white/95 [text-shadow:0_1px_10px_rgba(0,0,0,0.65)] md:text-base">
                Escápate a Taboga y disfruta un día de música en vivo, playa y buena energía.
              </p>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <Link href={eventDetailHref} className="w-full sm:w-auto">
      ...
      <section className="space-y-5">
        ...
        <TicketCarousel tickets={publicTickets} />
      </section>
        ...
        <div className="grid gap-5 md:grid-cols-2 md:items-start">
          <div className="flex flex-col rounded-2xl border border-[var(--border)] bg-white p-6 md:h-[360px]">
            ...
            <p className="mt-3 text-sm text-slate-600">
              Un evento diseñado para disfrutar salsa y vallenato en un ambiente frente al mar, con opciones de acceso
              por crucero y localidades para todos los gustos.
            </p>
